...
async def get_cached_response(
    ...
    try:
        ...
        # Check for vector dimension mismatch and reset index if needed
        if "vector blob size" in str(e):
            print(f"⚠️ Vector dimension mismatch detected. Resetting index {CACHE_INDEX_NAME}...")
            try:
                redis = await _get_redis()
                await redis.ft(CACHE_INDEX_NAME).dropindex(delete_documents=True)
                # Next call will recreate with correct dimension
            except:
                pass
